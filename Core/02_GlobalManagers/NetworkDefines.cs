...
namespace PahlUnity
{
    ...
    public class NetProtocol
    {
        ...
        static public byte[] ToArray(Header msg, byte[] body)
        {
            ...
            {
                msg.Length = HeadSize();
                buf.AddRange(BytesParser.Serialize(msg));
            }
            else
            {
        ...
        static public Header ToMessage(byte[] buf, out byte[] body)
        {
            ...
            Header msg = BytesParser.Deserialize<Header>(ref buf);
    ...
    public class PVPInfo : ByteSerializer
    {
        ...
        public byte[] Serialize()
        {
            ...
            if (pros != null)
            {
                foreach (ProductInfo info in pros)
                    rets.AddRange(BytesParser.Serialize(info));
            }
            return rets.ToArray();
        }
        ...
        public void Deserialize(byte[] bytes)
        {
            try
            {
                ...
                int proSize = BytesParser.Sizeof<ProductInfo>();
                for (; off < bytes.Length; off += proSize)
                    infos.Add(BytesParser.Deserialize<ProductInfo>(ref bytes, off));
